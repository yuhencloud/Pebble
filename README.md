...
- **Real-Time Status Monitor**: Displays `waiting` / `executing` states updated via Claude Code hooks
...
### Roadmap
- [ ] GUI-based permission approvals (respond to Claude Code permission requests from the panel)
...
```bash
git clone https://github.com/yourname/pebble.git
...
6. **Wait ~8 seconds after completion** — it turns yellow (`waiting`) and a native notification appears
...
**Status Inference**: `UserPromptSubmit` sets status to `executing`. If no hook events arrive for 8 seconds, the status returns to `waiting` and a system notification is fired.
